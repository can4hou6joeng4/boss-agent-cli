...
- [ ] abstract the authentication source into a `BrowserSessionProvider` seam (Issue #387): make "reuse the login session that already exists in the user's everyday browser" a first-class capability via `discover` / `verify` / `request` / `close`, hiding browser differences inside adapters and returning opaque, credential-free session references. **Only connect to an already-running, already-logged-in browser; never launch a new instance, never trigger a login, never silently fall back to CDP or headless** - re-authenticating in a separate CDP browser can invalidate the user's everyday session, so "launch CDP and log in again" is not an acceptable fallback. Landing in slices: the Chrome/Bridge read-only channel first (PR #388, adding `BROWSER_SESSION_NOT_FOUND`), with Edge/Firefox and multi-profile/container discovery tracked here.
- [ ] adapt to the mcp 2.x Server API (Issue #398): mcp 2.0 removed the `@server.list_tools()` and `@server.call_tool()` decorators in favour of `add_request_handler`. The dependency is currently capped at `mcp>=1.0.0,<2.0.0` (v1.19.1); the cap can only be lifted after the adaptation lands and the `fresh_install` CI job stays green.
