...
- Current platform coverage: `zhipin` has both candidate and recruiter implementations, but sensitive workflows are blocked by default; `zhilian` supports candidate-side read-only + local-assist parity, including `search --welfare`, `detail`, `show`, `history`, `me`, and `interviews`, while write/social workflows are blocked by default and the recruiter side is still unavailable; `qiancheng` / 51job is a registered placeholder adapter whose real workflows return `NOT_SUPPORTED`.
